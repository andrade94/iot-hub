...
export default function AlertRuleCreate({ site, devices, availableMetrics = [] }: Props) {
    const { t } = useLang();
    ...
    const form = useForm({
        ...
        device_id: '',
        active: true,
        conditions: [{ metric: '', condition: 'above' as string, threshold: '' as string, duration_minutes: '0' as string, severity: 'medium' }],
    });
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        const payload = {
            ...
            conditions: form.data.conditions.map((c) => ({
                ...
                severity: form.data.severity, // auto-set to rule severity
            })),
        };
        router.post(`/sites/${site.id}/rules`, payload);
    }
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${t('Create Alert Rule')} — ${site.name}`} />
                ...
                <div className="obsidian flex h-full flex-1 flex-col bg-background p-5 md:p-8">
                    ...
                    <FadeIn direction="down" duration={400}>
                        <div className="flex items-start justify-between">
                            <div>
                                <button type="button" onClick={() => router.get(`/sites/${site.id}/rules`)}
                                ...
                                <h1 className="font-display text-[28px] font-bold tracking-tight">{t('Create Alert Rule')}</h1>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <Button type="button" variant="outline" size="sm" className="text-[11px]"
                                    onClick={() => router.get(`/sites/${site.id}/rules`)}>{t('Cancel')}</Button>
                        ...
                        <div>
                                    ...
                                    <CardContent className="p-5 space-y-5">
                                        <div>
                                            ...
                                            <Input value={form.data.name} onChange={(e) => form.setData('name', e.target.value)}
                                                placeholder={t('e.g. Walk-in Cooler Max Temperature')} className="mt-1 text-[13px] font-medium" />
                                            {form.errors.name && <p className="mt-1 text-[10px] text-destructive">{form.errors.name}</p>}
                                        </div>
                                                ...
                                                <SelectContent>
                                                    ...
                                                    {devices.map((d) => (
                                                        <SelectItem key={d.id} value={String(d.id)}>
                                                            {d.name} <span className="text-muted-foreground">· {d.model}{d.zone ? ` · ${d.zone}` : ''}</span>
                                                        </SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                            ...
                            {form.data.conditions.map((cond, idx) => (
                                <div key={idx}>
                                    ...
                                    <FadeIn delay={idx * 40} duration={300}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="relative mt-1">
                                                            <Input value={cond.metric} onChange={(e) => updateCondition(idx, 'metric', e.target.value)}
                                                                placeholder="temperature" className="font-mono text-[11px]" list={`metrics-${idx}`} />
                        ...
                        <div>
                            <FadeIn delay={100} duration={400}>
                                <div className="sticky top-24">
                                    <div className="my-0 flex items-center gap-4 mb-4">
                                        <div className="h-px flex-1 bg-border" />
                                    ...
                                    <div className="rounded-lg border border-primary/15 bg-primary/[0.03] p-5">
                                        ...
                                        {form.data.conditions.map((cond, idx) => (
                                            <div key={idx}>
                                                ...
                                                <p className="text-[13px] leading-relaxed">
                                                    ...
                                                    {metricUnits[cond.metric] && <span className="text-[10px] text-muted-foreground ml-0.5">{metricUnits[cond.metric]}</span>}
                                        ...
                                        <div className="mt-4 border-t border-primary/10 pt-3 space-y-1 text-[11px] text-muted-foreground">
                                            ...
                                            <p>Status: <span className={form.data.active ? 'text-emerald-500' : 'text-muted-foreground'}>{form.data.active ? t('Active') : t('Inactive')}</span></p>
                                        </div>
                                    </div>

                                    <div className="mt-4 rounded-lg border border-border bg-card p-4">
                                        <p className="font-mono text-[9px] font-semibold tracking-[0.1em] text-muted-foreground/60 mb-2">{t('HOW IT WORKS')}</p>
                                        <ul className="space-y-1 text-[11px] text-muted-foreground">
                                            <li className="flex gap-2"><span className="text-primary">→</span> {t('All conditions must be met simultaneously')}</li>
                                            <li className="flex gap-2"><span className="text-primary">→</span> {t('Duration = 0 means trigger instantly')}</li>
                                            <li className="flex gap-2"><span className="text-primary">→</span> {t('Cooldown prevents repeated alerts')}</li>
                                        </ul>
                                    </div>
                                </div>
                            </FadeIn>
